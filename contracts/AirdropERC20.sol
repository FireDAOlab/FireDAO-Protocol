...
contract AirdropERC20 {
    ...
    event record(uint256 pid, string username, uint256 fid, address user, uint256 amount, uint256 time);
    ...
    constructor(FirePassport _fp,IERC20 _token,address _admin,address _fireSoul){
        token = _token;
        admin = _admin;
        fp = _fp;
        fireSoul  = _fireSoul;
    ...
    function addClaimList(address[] memory _user, uint256[] memory _amount) public {
        require(allowAddr[msg.sender], 'address error');
        for(uint256 i = 0 ; i < _user.length; i++) {
            userCanClaim[_user[i]]+=_amount[i];
        }
    }

    
    function Claim(uint256 _amount) public {
        require(_amount <= userCanClaim[msg.sender], '_amount error');
        if(_amount == userCanClaim[msg.sender]){
            token.transfer(msg.sender ,_amount);
            delete userCanClaim[msg.sender];
        }else {
            token.transfer(msg.sender ,_amount);
            userCanClaim[msg.sender] -= _amount;
        }
        emit record(checkPid(msg.sender) , checkUsername(msg.sender),IFireSoul(fireSoul).checkFIDA(msg.sender),msg.sender,_amount, block.timestamp );
    }
