...
contract cityNode is ERC1155, Ownable ,ReentrancyGuard {
    ...
    function cityNodeIncome(address _user, uint256 _income) external {
        ...
        uint256 userIndex = cityNodeUserNum[_user];
        cityNodeIncomeAmount[userIndex] = cityNodeIncomeAmount[userIndex].add(_income);
    }
    ...
    function getCityNodeReputation(uint256 cityNodeNum) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < cityNodeMember[cityNodeNum].length; i++) {
            // Use SafeMath's add function for addition
            cityNodeReputation = cityNodeReputation.add(IReputation(Reputation).checkReputation(cityNodeMember[cityNodeNum][i]));
        }
